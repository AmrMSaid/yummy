...
function displayMealDetails(meal) {
  ...
  for (let i = 1; i <= 20; i++) {
    if (meal[`strIngredient${i}`]) {
      ingredients += `<div class="alert alert-info m-2 p-1" role="alert">
  ...
  for (let i = 0; i < tagsArr.length; i++) {
    tags += `<div class="alert alert-light m-2 p-1 mb-5" role="alert">
                  ${tagsArr[i]}
                </div>`;
  }
  let temp = `
    ...
    <div class="col-md-8">
      ...
      <a class="btn btn-success m-1" href="${meal.strSource}" target="_blank">Source</a>
      <a class="btn btn-danger m-1" href="${meal.strYoutube}" target="_blank">YouTube</a>
    </div>  
  `;
